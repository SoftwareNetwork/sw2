...
#define WINAPI_CALL(x)                                                                                                 \
  if (!(x)) {                                                                                                          \
    throw ::sw::win32::winapi_exception{#x};                                                                           \
  }
#define WINAPI_CALL_HANDLE(x)                                                                                          \
  ::sw::win32::handle {                                                                                                \
    x, [] { throw ::sw::win32::winapi_exception{"bad handle from " #x}; }                                              \
  }
...
struct handle {
  ...
  handle(handle &&rhs) noexcept { operator=(std::move(rhs)); }
...
struct pipe {
  ...
  auto pipe_name() { return std::format(L"\\\\.\\pipe\\swpipe.{}.{}", GetCurrentProcessId(), pipe_id()); }
  void init(bool inherit_read, bool overlapped_read, bool inherit_write, bool overlapped_write) {
    ...
    DWORD buffer_size{};
    r = WINAPI_CALL_HANDLE(CreateNamedPipeW(s.c_str(),
                                            PIPE_ACCESS_INBOUND | (overlapped_read ? FILE_FLAG_OVERLAPPED : 0), 0, 1,
                                            buffer_size, buffer_size, 0, &sar));

    SECURITY_ATTRIBUTES saw{};
    saw.bInheritHandle = !!inherit_write;
    w = WINAPI_CALL_HANDLE(CreateFileW(s.c_str(), GENERIC_WRITE, 0, &saw, OPEN_EXISTING,
                                       FILE_ATTRIBUTE_NORMAL | (overlapped_write ? FILE_FLAG_OVERLAPPED : 0), 0));
  }
...
struct executor {
  ...
  void register_handle(auto &&h) { WINAPI_CALL(CreateIoCompletionPort(h, port, 0, 0)); }
  ...
  void read_async(auto &&h, void *data, size_t sz, auto &&user_f) {
    auto cb = new io_callback;
    cb->f = [cb, user_f = std::move(user_f)](size_t sz) mutable {
      cb->size = sz;
      if (sz == 0) {
        if (!cb->ec) {
          cb->ec = std::error_code{1, std::generic_category()};
        }
        user_f(cb);
        delete cb;
      } else {
        cb->ec = std::error_code{};
        user_f(cb);
      }
    };
    read_async(h, cb, data, sz);
  }
  void read_async(auto &&h, io_callback *cb, void *data, size_t sz) {
    cb->o = OVERLAPPED{};
    ++jobs;
    if (!ReadFile(h, data, sz, 0, &cb->o)) {
      auto err = GetLastError();
      if (err != ERROR_IO_PENDING) {
        --jobs;
        cb->ec = std::error_code{(int)err, std::generic_category()};
      }
    }
  }
...
inline void debug_break() { DebugBreak(); }
