...
auto getenv(auto &&key) { return std::getenv(key); }
...
struct abspath : path {
  ...
  abspath() = default;
  abspath(const base &p) : base{p} { init(); }
  abspath(auto &&p) : base{p} { init(); }
...
void write_file_once(const path &fn, const string &content, const path &lock_dir) {
  auto sha3 = [](auto &&d) { return digest<crypto::sha3<256>>(d); };
...
inline auto temp_sw_directory_path() { return fs::temp_directory_path() / "sw"; }
...
struct std::hash<::sw::abspath> {
  size_t operator()(const ::sw::abspath &p) { return std::hash<sw::path>()(p); }
};
