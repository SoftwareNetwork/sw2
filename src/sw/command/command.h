...
if [ $E -ne 0 ]; then echo "Error code: $E"; fi
  ...
  struct command_pipeline {
    using f_type = std::move_only_function<raw_command *(executor &, int)>;

    std::vector<f_type> commands;
    bool executed{};
    ...
    command_pipeline(command_pipeline &&) = default;
    command_pipeline(const command_pipeline &) = delete;
    ~command_pipeline() noexcept(false) {
      if (commands.empty()) {
        return;
      }
      auto &ex = default_executor();
      raw_command *last{};
      for (auto &&c : commands) {
        auto raw = c(ex, 0);
        if (last) {
    ...
    void add(auto &&v) {
      commands.emplace_back([&](auto &ex, int run) -> raw_command * {
        if (run) {
          v.run(ex);
        }
        return &v;
    ...
    void add2(auto &&v) {
      commands.emplace_back([v = std::move(v)](auto &ex, int run) mutable -> raw_command * {
        if (run) {
          v.run(ex);
        }
        return &v;
  ...
  void save(const path &dir, shell_type t = detect_shell()) {
    auto fn = dir / std::to_string(hash()) += visit(t, [](auto &&v) {
      return v.extension;
    });
    string s;
    s += visit(t, [](auto &&v) {
    ...
    if (!arguments.empty()) {
      s.resize(s.size() - 2 - string{visit(t, [](auto &&v) {
                                return v.arg_delim;
                              })}.size());
      s += " "s + visit(t, [](auto &&v) {
             return v.any_arg;
  ...
  auto operator()(this auto &&self, auto &&arg, auto &&...args) {
    auto c = self;
    c += arg;
    ((c += args), ...);
    command_pipeline p;
...
// inherit to be compatible?
struct lambda_command : io_command {
  std::move_only_function<int(const std::vector<string> &)> f;
};
