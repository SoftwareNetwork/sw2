...
struct command_stream {
  ...
  command_stream(const command_stream &rhs) {
    this->s = rhs.s;
    this->h = rhs.h;
  }
  ...
  auto pre_create_command(auto os_handle, auto &&ex) {
    visit(
        s, [&](inherit) { h = GetStdHandle(os_handle); },
        [&](close_) {
          // empty
        },
        [&](path &fn) {
          ...
          h = WINAPI_CALL_HANDLE(CreateFileW(fn.wstring().c_str(), Output ? GENERIC_WRITE : GENERIC_READ, 0, &sa,
    ...
    visit(
        ...
        [&](string &s) {
          if constexpr (Input) {
            ...
            ex.write_async(pipe.w, s.data(), s.size(), [&, pos = 0ULL](auto &&cb) mutable {
              if (!cb->ec) {
                pos += cb->size;
            ...
            h = default_handle_value;
            s.resize(4096);
            ex.read_async(pipe.r, s.data(), s.size(), [&, pos = 0ULL](auto &&cb) mutable {
              if (!cb->ec) {
                pos += cb->size;
                if (pos == s.size()) {
  ...
  void finish(auto &&ex) {
    //pipe = decltype(pipe){};
    visit(
        s, [&](inherit) { h = default_handle_value; },
        [&](close_) {
          // empty
        },
        [&](string &s) {
...
struct raw_command {
  ...
  struct process_data {
    ...
    process_data() { si.StartupInfo.cb = sizeof(si); }
    process_data(const process_data &) { si.StartupInfo.cb = sizeof(si); }
    process_data &operator=(const process_data &) { return *this; }
    void close() { *this = process_data{}; }
  };
  process_data d;
  //
  bool detach{};
  ...
  auto printw() const {
    ...
    for (auto &&a : arguments) {
      ...
      visit(a, overload{[&](const string &s) { quote(path{s}.wstring()); },
                        [&](const string_view &s) { quote(path{string{s.data(), s.size()}}.wstring()); },
                        [&](const path &p) { quote(p.wstring()); }});
    }
  ...
  std::string print() const {
    ...
    for (auto &&a : arguments) {
      ...
      visit(a, overload{[&](const string &s) { quote(s); },
                        [&](const string_view &s) { quote(string{s.data(), s.size()}); },
                        [&](const path &p) { quote(p.string()); }});
    }
    return s;
  }
  ...
  void run_platform(auto &&ex, auto &&cb) {
    ...
    d.si.StartupInfo.dwFlags = STARTF_USESTDHANDLES; // this does not make sense with PROC_THREAD_ATTRIBUTE_HANDLE_LIST
    if (detach) {
    ...
    scope_exit seh{[&] { HeapFree(GetProcessHeap(), 0, d.si.lpAttributeList); }};
    WINAPI_CALL(InitializeProcThreadAttributeList(d.si.lpAttributeList, 1, 0, &size));
    scope_exit sel{[&] { DeleteProcThreadAttributeList(d.si.lpAttributeList); }};
    ...
    WINAPI_CALL(CreateProcessW(prog.data(), cmd.data(), 0, 0, inherit_handles, flags, env,
                               wdir.empty() ? 0 : wdir.data(), (LPSTARTUPINFOW)&d.si, &d.pi));
    ...
    // won't stop.
    ex.register_process(d.pi.hProcess, d.pi.dwProcessId,
                        [this, &ex, cb, process_job = std::move(process_job)](bool time_limit_hit) {
                          if (time_limit_hit) {
  ...
  void add(const char *p) { arguments.push_back(string{p}); }
  auto operator+=(auto &&arg) {
    add(arg);
    return appender{[&](auto &&v) { add(v); }};
  }

  void operator<(const path &p) { in = p; }
  void operator>(const path &p) { out = p; }
  ...
  void operator||(const raw_command &c) { SW_UNIMPLEMENTED; }
  void operator&&(const raw_command &c) { SW_UNIMPLEMENTED; }

  bool ok() const { return exit_code && *exit_code == 0; }
